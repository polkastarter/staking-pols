...
// set polsStake_address , then run this script ...
// $ MAINNET_PRIVATE_KEY=e15ffb5... npx hardhat run scripts/verify-bsc.ts --network bscMain

const LOCK_TIME_PERIOD = 7 * 24 * 60 * 60; // default 7 days
...
async function main(): Promise<void> {
  ...
  const polsStake_address = "0xD558675a8c8E1fd45002010BaC970B115163dE3a"; // BSC-Mainnet

  console.log("PolsStake deployed to: ", polsStake_address);
  ...
  */
  ...
  if (process.env.BSCSCAN_API_KEY !== undefined && process.env.BSCSCAN_API_KEY.length == 34) {
    await hre.run("verify:verify", {
      address: polsStake_address,
      constructorArguments: constructorArgs,
    });
  } else {
    console.log("Can not verify contract on BscScan - no BSCSCAN_API_KEY");
  }
}
